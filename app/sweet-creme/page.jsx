...
const SweetCremeReceipt = () => {
    const [billNo, setBillNo] = useState(4059);
    const [date, setDate] = useState("");
    const [time, setTime] = useState("");
    const [items, setItems] = useState([]);
    const [tokenNo, setTokenNo] = useState(2200);
    const [numItems, setNumItems] = useState(); // ✅ Added missing state
        ...
        <div className="flex flex-col justify-center items-center min-h-screen bg-gray-100 p-4">
            ...
            <div className="bg-white w-[360px] p-4 text-sm font-mono shadow-lg">
                ...
                <div className="text-center pb-2">
                    <Image src={sweetCreme} width={150} alt="sweet Logo" className="mx-auto" priority />
                ...
                <div className="mt-2 text-md pb-2">
                    <p><span className="font-bold">Bill#:</span> <span>{billNo}</span></p>
                    <p><span className="font-bold">Date:</span> {formatDate(date)}</p>
                    <p><span className="font-bold">Time:</span> {formatTime(time)}</p>
                </div>
                ...
                <div className="mt-5 pb-2">
                    <p className="flex justify-between"><span className="font-bold">Gross Amt : </span><span>{grossAmount}</span></p>
                    <p className="flex justify-between"><span className="font-bold">Net Total :</span> <span>{grossAmount}</span></p>
                    <p className="flex justify-between"><span className="font-bold">Total Amount :</span> <span>{grossAmount}</span></p>
                </div>

                {/* Payment Info */}
                <div className="mt-2 pb-2">
                    <p className="flex justify-between"><span className="font-bold">Credit Card Payment:</span> <span>{grossAmount}</span></p>
                </div>
